...
@interface ShaiDanViewController ()<LifeZoneListTableViewCellDelegate>
{
    int pageNum;
...
-(void)httpShaiDan
{
     HttpHelper *helper = [[HttpHelper alloc] init];
    __weak ShaiDanViewController *weakSelf = self;
    [helper loadProductdetailproductId:_goodId pageNum:[NSString stringWithFormat:@"%d",pageNum] limitNum:@"20" success:^(NSDictionary *resultDic) {
        [self hideRefresh];
        if ([[resultDic objectForKey:@"status"] integerValue] == 0) {
            [weakSelf handleloadResult:resultDic];
...
- (void)clickImageToSeeTableIndex:(NSIndexPath *)tindex image:(UIImageView *)image collectIndex:(NSIndexPath *)cindex
{
    ...
    [PhotoBroswerVC show:self type:PhotoBroswerVCTypeZoom index:cindex.row photoModelBlock:^NSArray *{
        NSArray *array = [info.imgs componentsSeparatedByString:@","];
