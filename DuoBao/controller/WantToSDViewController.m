...
- (void)httpUpdateImage:(UIImage*)image index:(NSInteger)index
{
   ...
   // [self addImage:image addImage1:[UIImage imageNamed:@"zhushou(1)"]];
    [helper postImageHttpWithImage:[self addImage:image addImage1:[UIImage imageNamed:@"shaidan"]]
...
- (UIImage *)addImage:(UIImage *)useImage addImage1:(UIImage *)addImage1
{
    ...
    [addImage1 drawInRect:CGRectMake(useImage.size.width/3+15,useImage.size.height/2, addImage1.size.width, addImage1.size.height)];
...
{
    if (_titleTextView.text.length < 1) {
        [Tool showPromptContent:@"请输入标题" onView:self.view];
        return;
    }
...
- (void)clickPitctureTosee:(UITapGestureRecognizer*)tap
{
    ...
    [PhotoBroswerVC show:self type:PhotoBroswerVCTypeZoom index:indexPath.row photoModelBlock:^NSArray *{
